    ...
    public void GenerateMap ()
    {
        ...
        for (int i = 0; i < obstacleCount; i++)
        {
			...
			if (randomCoord != currentMap.mapCenter && MapIsFullyAccessible (obstacleMap, currentObstacleCount))
			{
				...
				Renderer obstaceRenderer = newObstacle.GetComponent <Renderer> ();
				Material obstacleMaterial = new Material (obstaceRenderer.sharedMaterial);

				float colorPercent = randomCoord.y / (float)currentMap.mapSize.y;
				obstacleMaterial.color = Color.Lerp (currentMap.foregroundColor, currentMap.backgroundColor, colorPercent);
				obstaceRenderer.sharedMaterial = obstacleMaterial;
		...
		navmeshFloor.localScale = new Vector3 (maxMapSize.x * tileSize, maxMapSize.y * tileSize, 1f);
    }
